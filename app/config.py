...
class Settings:
    ...
    # a secret access token so it can't run from the browser). The same
    # pixel ID is used for both — Reddit's CAPI endpoint is scoped by
    # pixel, not by ad account. Both from Reddit Ads Manager's Conversions
    # setup page (Events Manager -> pixel -> Conversions API). Either
    # empty means both are simply omitted, same convention as the Google
    # Ads/GA settings above.
